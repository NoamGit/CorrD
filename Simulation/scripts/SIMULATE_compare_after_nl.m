...
for n = 1:c % build c processes
    ...
    theta3_spc = linspace(125,150, 200);
...
%     amp = 60;
    ...
    assert(nl_image>0, 'Non linearity is not def positiv');
    assert(numel(spiketimes)/N>5, 'mean firing rate is to low');
    %% iteration plot
...
%     numel(syntData(4).spiketimes)/N
    ...
    linKernel_orig = cellfun(@(x) amp * x.val, {syntData.linKernel},'UniformOutput', false);
    ...
    % poly
    poly_order = 2;
    process = process.nlestimation(poly_order, 0, amp,'poly', 'resampled stimulus','Synt Data'); % stimulus has same dt as procees -> resampled
    % plot
    ...
    % exp
    poly_order = 0;
    process = process.nlestimation(poly_order,0, amp,'exp', 'resampled stimulus','Synt Data'); % stimulus has same dt as procees -> resampled
    % plot
