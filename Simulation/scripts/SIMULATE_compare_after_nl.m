...
for n = 1:c % build c processes
    ...
    dt_kernel = 1/30;
    model_kernel = @(t) exp(-(t-p_kernel(2)).^2/p_kernel(1)^2).*sin( p_kernel(4) *(t-p_kernel(3)) );
    t_kernel = linspace(-30*dt_kernel, 5*dt_kernel, 35/dt_stim); 
...
%     figure(2); plot(nl_domain, nl_image);% show nl   
    ...
    datainstance = struct('linKernel',kernel,'nonlinKernel',nonLinkernel,'CGP',CGP,'lambd',lambda,'spiketimes',spiketimes);  
    syntData = [syntData  datainstance];
end
    %% store generated data in process object 
    
    % TODO - add syntetic case in Load data
    kernNAME = 'gaussSine';
